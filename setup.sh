...
if [ -d "$TOOLS_DIR" ] || [ -d "$DESKTOP_DIR/Stand-Alone-1" ] || [ -d "$DESKTOP_DIR/Stand-Alone-2" ] || [ -d "$DESKTOP_DIR/Stand-Alone-3" ] || [ -d "$DESKTOP_DIR/AD-set" ]; then
    ...
    echo "${ORANGE}[*]${NC}Please remove or rename the existing folders before running this script again."
    exit 1
fi
...
read choice
case "$choice" in 
  yes|y ) echo "${ORANGE}[*]${GREEN}Continuing${NC} with the script...";;
  no|n ) echo "${ORANGE}[*]${RED}Exiting script."; exit 1;;
  * ) echo "${ORANGE}[*]${RED}Invalid response. Exiting script.${NC}"; exit 1;;
esac
...
sudo apt install golang-go autorecon seclists curl dnsrecon enum4linux feroxbuster gobuster impacket-scripts nbtscan nikto nmap onesixtyone oscanner redis-tools smbclient smbmap snmp sslscan sipvicious tnscmd10g whatweb wkhtmltopdf -y -qq > /dev/null 2>&1
echo ""
...
countdown() {
    for i in {10..1}; do
        echo -n "${ORANGE}[*]${NC} Exam setup ${GREEN}completed${NC} Script will end in ${ORANGE}$i${NC} seconds..."
        sleep 1
...
exec zsh
echo "${ORANGE}[*]${NC} Clearing the screen..."
clear
